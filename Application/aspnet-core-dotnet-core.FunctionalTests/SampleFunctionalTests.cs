    ...
    public class SampleFunctionalTests
    {
        ...
        public void SampleFunctionalTest3()
        {
            ...
            while (true)
            {
                ...
                {
                    driver.Navigate().GoToUrl(webAppUrl);
                    Assert.AreEqual("Your ASP.NET Core app is up and running on Azure Staging WebApp V7.0.0", driver.FindElementByCssSelector("div[class='description line-2']").Text, "Expected description line  to be 'Your ASP.NET Core app is up and running on Azure Staging WebApp V6.0.0'");
                    break;
                }
                catch(Exception e)
                {
